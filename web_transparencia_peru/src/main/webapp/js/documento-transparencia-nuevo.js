...
$(document).ready(function () {
    ...
    if (window.documentoTransparenciaInitialized) {
        console.log("Manejadores ya inicializados, omitiendo...");
        return;
    }
    window.documentoTransparenciaInitialized = true;

    console.log("Inicializando manejadores para documentos de transparencia");

    // Función auxiliar para obtener el path del contexto
    function getContextPath() {
        return $('meta[name="context-path"]').attr('content') || '';
    }

    // Reinicializar manejadores para evitar duplicados
    $('#verDocumentoModal').off('show.bs.modal');
    $('#editarDocumentoModal').off('show.bs.modal');
    $('#editarDocumentoModal').off('hidden.bs.modal');

    // Desactivar todos los manejadores existentes primero para evitar conflictos
    $(document).off('show.bs.modal', '#verDocumentoModal');
    $(document).off('shown.bs.modal', '#verDocumentoModal');

    // Este es un fix crítico: detener cualquier otro handler existente en el DOM
    // que pueda estar sobrescribiendo nuestros datos
    const originalBootstrapModal = $.fn.modal;
    $.fn.modal = function (option) {
        const result = originalBootstrapModal.apply(this, arguments);
        if (this.is('#verDocumentoModal') && option === 'show') {
            // Asegurar que el modal no sea manipulado después de mostrado
            setTimeout(function () {
                $('#verDocumentoModal').off('shown.bs.modal.document-reset');
            }, 100);
        }
        return result;
    };
    ...
    $('#verDocumentoModal').on('show.bs.modal', function (e) {
        ...
        if (documentoId && documentoId > 0) {
            ...
            console.log("URL de petición:", ajaxUrl);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if ($('#debugDocumentoInfo').length) {
                        $('#debugDocumentoInfo').text(JSON.stringify(data, null, 2));
                    }

                    // Fix para evitar que se sobrescriban los datos
                    setTimeout(function () {
                        console.log("Verificando que los datos sigan visibles");
                        if ($('#detallesDocumento').hasClass('d-none')) {
                            console.warn("Los datos fueron ocultados por otro script, corrigiendo...");
                            $('#loadingDocumento').addClass('d-none');
                            $('#detallesDocumento').removeClass('d-none');
                        }
                    }, 500);
    ...
    // Fix adicional: observar cambios en el DOM para restaurar datos si son eliminados
    const targetNode = document.getElementById('verDocumentoModalBody');
    if (targetNode) {
        const config = {attributes: true, childList: true, subtree: true};
        const callback = function (mutationsList, observer) {
            for (let mutation of mutationsList) {
                if (mutation.type === 'attributes' &&
                    mutation.target.id === 'detallesDocumento' &&
                    mutation.attributeName === 'class') {

                    const isHidden = $('#detallesDocumento').hasClass('d-none');
                    const isLoading = !$('#loadingDocumento').hasClass('d-none');

                    // Si los detalles están ocultos pero no estamos en carga, algo está mal
                    if (isHidden && !isLoading) {
                        console.warn("Detección de manipulación externa del DOM, restaurando vista de detalles");
                        $('#loadingDocumento').addClass('d-none');
                        $('#detallesDocumento').removeClass('d-none');
                    }
                }
    ...
    // Sobrescribir cualquier otro manejador que pudiera estar en transparencia.jsp
    // Esto es importante porque transparencia.jsp podría tener sus propios manejadores
    setTimeout(function () {
        console.log("Verificando conflictos de eventos...");

        // Crear un sumidero de eventos para capturar cualquier otro evento
        $(document).on('show.bs.modal', '#verDocumentoModal', function (e) {
            console.log("Evento show.bs.modal de documento transparencia capturado en el nivel document");
            e.stopPropagation();
        });

        // Verificar si el modal está funcionando correctamente
        $('#verDocumentoModal').on('shown.bs.modal', function () {
            console.log("Modal mostrado - verificando estado");
            setTimeout(function () {
                if ($('#detallesDocumento').hasClass('d-none') &&
                    $('#loadingDocumento').hasClass('d-none')) {
                    console.error("Estado inválido del modal detectado: ambos elementos ocultos");
                    $('#detallesDocumento').removeClass('d-none');
                }
            }, 100);
        });
    ...
    // Función pública para inicializar visualizador de documentos (para usar desde la consola si es necesario)
    window.initDocumentoViewer = function (documentoId) {
        if (documentoId) {
            // Simulamos click en un botón para abrir el modal
            const button = $('<button>').data('id', documentoId);
            const modal = $('#verDocumentoModal');

            // Disparamos el evento manualmente
            const event = $.Event('show.bs.modal');
            event.relatedTarget = button[0];
            modal.trigger(event);
            modal.modal('show');
