...
<head>
    ...
    <title>Presupuesto por Proyectos | Portal de Transparencia Perú</title>
    <!-- Bootstrap CSS -->
                ...
                <div class="search-container position-relative w-100">
                    ...
                    <div class="search-tags d-none d-md-flex justify-content-center mt-2">
                        <span class="badge bg-light text-dark mx-1" style="cursor: pointer;"><i
                                class="fas fa-tag me-1"></i>Transporte</span>
                        <span class="badge bg-light text-dark mx-1" style="cursor: pointer;"><i
...
<div class="hero-section">
    ...
    <div class="container position-relative">
        <div class="row align-items-center">
            <div class="col-lg-6">
                <h1 class="display-5 fw-bold mb-3">Proyectos de Inversión</h1>
                <p class="lead">Explore los principales proyectos de inversión pública del Estado Peruano, su
                    presupuesto asignado, avance y entidades responsables.</p>
            </div>
            <div class="col-lg-6">
        ...
        <div class="col-12">
            <div class="info-box">
                ...
                <div class="d-flex flex-wrap gap-2">
                    ...
                    <a href="ServletPresupuesto?accion=proyectos" class="btn btn-primary">Por Proyectos</a>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-2">
                                <select class="form-select" id="regionSelect" name="region">
                                    ...
                                    <option value="Lima" ${regionSeleccionada == 'Lima' ? 'selected' : ''}>Lima</option>
                            ...
                            <div class="col-md-4 mb-2">
                                <select class="form-select" id="sectorSelect" name="sector">
                                    ...
                                    <option value="Educación" ${sectorSeleccionado == 'Educación' ? 'selected' : ''}>
                                        Educación
                                    </option>
                                    <option value="Salud" ${sectorSeleccionado == 'Salud' ? 'selected' : ''}>Salud
                                    </option>
                                    <option value="Transporte" ${sectorSeleccionado == 'Transporte' ? 'selected' : ''}>
                                        Transporte
                                    </option>
                                    <option value="Vivienda" ${sectorSeleccionado == 'Vivienda' ? 'selected' : ''}>
                                        Vivienda
                                    </option>
                                    <!-- Más sectores -->
                                </select>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">Principales Proyectos de Inversión
                ...
                <div class="card-body">
                    <div class="chart-container chart-animate mb-4">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover">
                            ...
                            <tbody>
                            <c:choose>
                                <c:when test="${not empty datosProyectos}">
                                    <c:forEach var="proyecto" items="${datosProyectos}" varStatus="status">
                                        <tr>
                                            <td>${proyecto.nombre}</td>
                                            <td>
                                                <fmt:formatNumber value="${proyecto.presupuestoAsignado / 1000000}"
                                                                  pattern="#,##0.0"/>
                                            </td>
                                            <td>${proyecto.entidadNombre}</td>
                                            <td>${proyecto.region}</td>
                                            <td>
                                                <div class="progress">
                                                    <div class="progress-bar ${proyecto.avanceFisico >= 70 ? 'bg-success' : proyecto.avanceFisico >= 40 ? 'bg-info' : 'bg-warning'}"
                                                         style="width: ${proyecto.avanceFisico}%">
                                                        <fmt:formatNumber value="${proyecto.avanceFisico}"
                                                                          pattern="#,##0.0"/>%
                                                    </div>
                                                </div>
                                            </td>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        <c:forEach var="proyecto" items="${datosProyectos}" varStatus="status">
        proyectos.push("${proyecto.nombre}");
        presupuestos.push(${proyecto.presupuestoAsignado / 1000000});
        avances.push(${proyecto.avanceFisico});
        backgroundColors.push(coloresFondo[${status.index % 7}]);
        borderColors.push(coloresBorde[${status.index % 7}]);
        </c:forEach>

        // Si hay más de 5 proyectos, tomar solo los 5 primeros para el gráfico
        const maxProyectos = 5;
        if (proyectos.length > maxProyectos) {
            proyectos.length = maxProyectos;
            presupuestos.length = maxProyectos;
            avances.length = maxProyectos;
            backgroundColors.length = maxProyectos;
            borderColors.length = maxProyectos;
        }

        // Inicializar gráfico de proyectos
        const ctxProyectos = document.getElementById('proyectosChart').getContext('2d');

        // Destruir gráfico si ya existe
        if (window.proyectosChart instanceof Chart) {
            window.proyectosChart.destroy();
        }

        if (proyectos.length > 0) {
            window.proyectosChart = new Chart(ctxProyectos, {
                type: 'bar',
                data: {
                    labels: proyectos,
                    datasets: [{
            ...
            errorMessage.innerHTML = '<i class="fas fa-exclamation-triangle me-2"></i>No hay datos disponibles para visualizar los proyectos.';
